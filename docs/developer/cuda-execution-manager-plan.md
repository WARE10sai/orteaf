...
Note: The CUDA managers should live in the execution layer (mirroring MPS/CPU).

## Goal
Introduce a CUDA execution manager layer under `internal/execution/cuda/manager` that owns
CUDA resources (device/context/stream/event) and exposes a clean lifecycle API similar
to the existing MPS/CPU managers.

## Target File Layout
- `orteaf/include/orteaf/internal/execution/cuda/manager/`
  - `cuda_execution_manager.h`
  - `cuda_device_manager.h`
  - `cuda_buffer_manager.h`
  - `cuda_context_manager.h`
  - `cuda_module_manager.h`
  - `cuda_function_manager.h`
...
- `CudaFunctionManager`
  - Resolves CUDA functions from a module and manages cached handles.
- `CudaStreamManager`
...
- `CudaModuleManager` -> per-module resource that owns a function manager.
...
2. Add headers for CUDA execution managers with configs and public APIs.

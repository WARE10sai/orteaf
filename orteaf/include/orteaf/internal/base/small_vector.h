 ...
 *
 * SmallVector keeps up to N elements inline before falling back to heap
 * storage. It provides a std::vector-like API with contiguous storage
 * semantics.
 *
 * Exception safety:
...
public:
  ...
  /// @brief Constructs an empty SmallVector using inline storage when
  /// available.
  SmallVector() noexcept { resetInlinePointer(); }
  ...
  /// @brief Replaces contents with count copies of value (copy-constructible
  /// only).
  ...
  /// @brief Replaces contents with an initializer list (copy-constructible
  /// only).
        ...
        try {
          for (; i < size_; ++i) {
            ::new (static_cast<void *>(data_ + i))
                T(std::move_if_noexcept(old_data[i]));
          }
        } catch (...) {
          destroyRange(data_, data_ + i);
          if (was_heap) {
  ...
  /// @brief Inserts an element at pos by forwarding args (nothrow move
  /// required).
  ...
  /// @brief Erases elements in [first, last) and returns iterator to next
  /// element.
  ...
  void
  swap(SmallVector &other) noexcept(std::is_nothrow_swappable_v<T> &&
                                    std::is_nothrow_move_constructible_v<T> &&
                                    std::is_nothrow_move_assignable_v<T>) {
    if (this == &other) {
      return;
...
private:
  ...
  void moveFrom(SmallVector &other) {
    if (other.usingHeapStorage()) {
      ...
      try {
        for (; i < other.size_; ++i) {
          ::new (static_cast<void *>(data_ + i))
              T(std::move_if_noexcept(other.data_[i]));
        }
      } catch (...) {
        destroyRange(data_, data_ + i);
        throw;
  ...
  void ensureCapacity(size_type min_capacity) {
    ...
    size_type new_capacity =
        capacity_ ? capacity_ * 2
                  : (has_inline_storage ? stack_capacity_value : size_type{0});
    if (new_capacity == 0) {
      new_capacity = 1;
    }
  ...
  static void deallocate(pointer ptr) noexcept { ::operator delete(ptr); }
  ...
  void shiftRight(size_type index, size_type count) {
    ...
    for (size_type i = size_; i > index; --i) {
      ...
      ::new (static_cast<void *>(data_ + dest))
          T(std::move_if_noexcept(data_[src]));
      data_[src].~T();
    }
  }
...
void swap(SmallVector<T, N> &lhs,
          SmallVector<T, N> &rhs) noexcept(noexcept(lhs.swap(rhs))) {
  lhs.swap(rhs);
}
